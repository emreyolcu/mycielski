...
const char * Parser::parse_dimacs_non_profiled (int & vars, int strict) {
  ...
  if (ch == 'c')
    ...
    {
      if (!incremental)
	PER ("unexpected 'i' after 'p ' in incremental mode");
      *incremental = true;
  ...
  if ((!incremental || !*incremental) && parsed < clauses && strict > 0)
    PER ("clause missing");
...
#ifndef QUIET
  if ((incremental && *incremental) || num_cubes)
    {
      double end = internal->time ();
