...
// ('false') but is set to '1' by the stand alone solver.  Using here a
// static default value avoids that the stand alone solver reports that
// '--report=1' is different from the default in 'print ()' below.
//
int Options::report_default_value;

// This makes printing the string for the default value of 'report'
// unfortunately very complicated and we use this function to hide
// this complex logic in the usage of the 'OPTIONS' macro below.

const char *
Options::default_value_string (char * buffer,
                               const int * ref, const char * str)
{
  Options * tmp = 0;
  if (ref == &tmp->report)
    {
      sprintf (buffer, "%d", report_default_value);
      return buffer;
    }
  return str;
}

#define DEFAULT_VALUE_STRING(N,V) \
  default_value_string (default_value_string_buffer, \
                        &((Options*)0)->N, #V)
...
Options::Options (Internal * s) : internal (s)
{
  ...
  //
  const char * prev = "";
  char default_value_string_buffer[16];
  size_t i = 0;
# define OPTION(N,V,L,H,O,D) \
  do { \
    if ((L) > (V)) \
